...
void calculate_offsets(int img_w, int img_h, int canvas_w, int canvas_h,
                       Align align, int &ox, int &oy) {
...
std::vector<float> align_mono_sdf(const RasterResultF32 &res, int canvas_w,
                                  int canvas_h, Align align) {
...
std::vector<float> align_color_sdf(const RasterResultF32 &res, int canvas_w,
                                   int canvas_h, Align align) {
...
int main() {
  ...
  if (!sdf_raw_A.error.empty()) {
    std::cerr << "SDF A Error: " << std::string(sdf_raw_A.error) << std::endl;
    return 1;
  }

  auto sdf_raw_B = render_typst_to_sdf_f32(typst_B_mono, target_dpi);
  if (!sdf_raw_B.error.empty()) {
    std::cerr << "SDF B Error: " << std::string(sdf_raw_B.error) << std::endl;
    return 1;
  }

  auto sdf_mono_A =
      align_mono_sdf(sdf_raw_A, CANVAS_W, CANVAS_H, Align::Center);
  auto sdf_mono_B =
      align_mono_sdf(sdf_raw_B, CANVAS_W, CANVAS_H, Align::Center);
  ...
  for (int i = 0; i < num_frames; ++i) {
    ...
    ss << "frames/mono/frame_" << std::setw(2) << std::setfill('0') << i
       << ".png";
...
#cetz.canvas({
  ...
  if (!raw_color_A.error.empty()) {
    std::cerr << "Color SDF A Error: " << std::string(raw_color_A.error)
              << std::endl;
    return 1;
  }

  auto raw_color_B = render_typst_to_rgba_esdt_f32(typst_B_color, target_dpi);
  if (!raw_color_B.error.empty()) {
    std::cerr << "Color SDF B Error: " << std::string(raw_color_B.error)
              << std::endl;
    return 1;
  }

  auto sdf_color_A =
      align_color_sdf(raw_color_A, CANVAS_W, CANVAS_H, Align::Center);
  auto sdf_color_B =
      align_color_sdf(raw_color_B, CANVAS_W, CANVAS_H, Align::Center);
  ...
  for (int i = 0; i < num_frames; ++i) {
    ...
    for (size_t p = 0; p < np_color; ++p) {
      ...
      frame_buffer_color[p4 + 0] =
          static_cast<uint8_t>(std::clamp(rp * 255.0f, 0.0f, 255.0f));
      frame_buffer_color[p4 + 1] =
          static_cast<uint8_t>(std::clamp(gp * 255.0f, 0.0f, 255.0f));
      frame_buffer_color[p4 + 2] =
          static_cast<uint8_t>(std::clamp(bp * 255.0f, 0.0f, 255.0f));
      frame_buffer_color[p4 + 3] =
          static_cast<uint8_t>(std::clamp(alpha * 255.0f, 0.0f, 255.0f));
    }

    std::ostringstream ss;
    ss << "frames/color/frame_" << std::setw(2) << std::setfill('0') << i
       << ".png";
